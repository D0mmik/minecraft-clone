...
import { BlockType, isTransparent, isSolid, isLiquid, getBlockTexture } from './BlockType.js';
import { UV_TABLE } from './TextureAtlas.js';
...
function computeAO(side1, side2, corner) {
...
function getBlockAt(chunk, x, y, z, getNeighborBlock) {
  ...
  return getNeighborBlock(worldX, y, worldZ);
}

function isSolidAt(chunk, x, y, z, getNeighborBlock) {
  return isSolid(getBlockAt(chunk, x, y, z, getNeighborBlock));
}

export function buildChunkMesh(chunk, getNeighborBlock) {
    ...
    for (let z = 0; z < CHUNK_SIZE; z++) {
      ...
      for (let x = 0; x < CHUNK_SIZE; x++) {
        ...
        for (let fi = 0; fi < 6; fi++) {
          ...
          let vCount, posOff, uvOff, idxOff;
          if (blockIsWater) {
...
function computeFaceAO(chunk, x, y, z, faceInfo, getNeighborBlock) {
  ...
  for (let vi = 0; vi < 4; vi++) {
    ...
    let side1Solid, side2Solid, cornerSolid;
