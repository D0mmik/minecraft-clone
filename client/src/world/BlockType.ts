...
function setTex(bt: number, top: number, side: number, bottom: number): void {
...
export function getBlockTexture(blockType: number, faceId: FaceId): number {
  return TEXTURE_LUT[blockType * 3 + faceId];
}

export function isTransparent(blockType: number): number {
  return TRANSPARENT_LUT[blockType];
}

export function isSolid(blockType: number): number {
  return SOLID_LUT[blockType];
}

export function isLiquid(blockType: number): number {
  return LIQUID_LUT[blockType];
}

// Block names for HUD display
export const blockNames: Record<number, string> = {
...
export const hotbarBlocks: number[] = [
  BlockType.GRASS,
  BlockType.DIRT,
  BlockType.STONE,
